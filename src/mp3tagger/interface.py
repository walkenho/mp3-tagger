...
def update_based_on_previous_value(
    songs: MP3Table, category: str, options=None
) -> None:
    if options:
        selection = st.sidebar.selectbox(
            f"Update {category}",
            options=songs.get_entries(category)
            + ["Select from Options", "Update Manually"],
        )
    else:
        selection = st.sidebar.selectbox(
            f"Update {category}",
    ...
    if selection == "Select from Options":
        ...
        songs.data[category] = st.sidebar.text_input(
            f"Provide a new entry for {category}", ""
        )
    else:
        songs.data[category] = selection


def get_albumpath_from_interface() -> Union[Path, None]:
