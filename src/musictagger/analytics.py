...
import pandas as pd
from musictagger.core import find_all_mp3s, load_mp3, BASEPATH
import logging
...
def collect_data(path: Path):
    ...
    for filename in all_mp3s:
        ...
        if audio is None:
            ...
            tags.append({**{k: v[0] for k, v in audio.items()},
                         'filename': str(filename)})
            if not audio:
                n_untagged = n_untagged + 1

    return tags, {'n_skipped': n_skipped, 'n_untagged': n_untagged, 'n_total': files_total}
...
def run_analytics():
    ...
    with open(DATAFOLDER/f'details_{timestamp}.txt', 'w') as f:
        ...
        for entry in BASEPATH.glob('*'):
            if entry.is_dir():
                ...
                if c['n_untagged'] !=0:
                    f.writelines(f"{c['n_untagged']} files without tags in this folder.\n")
    ...
    with open(DATAFOLDER/f'summary_{timestamp}.txt', 'w') as f:
        ...
        f.writelines(f"Out of the ingested, {df_counters['n_untagged'].sum()} completely untagged\n")
